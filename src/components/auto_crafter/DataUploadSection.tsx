...
export const DataUploadSection: React.FC<DataUploadProps> = ({
  ...
  const [uploading, setUploading] = useState<boolean>(false);
    ...
    const downloadPdfReport = async (datasetName: string): Promise<Blob> => {
        try {
        const response = await fetch(`auto/generate-pdf?dataset_name=${encodeURIComponent(datasetName)}`, {
            method: "POST",
        });
        ...
        throw new Error("Error downloading PDF report");
        }
    };
  ...
  const downloadPdf = async () => {
    ...
    try {
      const pdfBlob = await downloadPdfReport(file.name);
